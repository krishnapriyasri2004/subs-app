                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {product.exclusive && (
                                                        <Badge className="h-4 px-1.5 text-[8px] font-black uppercase tracking-widest bg-primary/10 text-primary border-none">Exclusive</Badge>
                                                    )}
                                                </div>
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-[160px] rounded-xl border-border/50 shadow-2xl glass-card">
                                                ...
                                                {product.status !== 'archived' ? (
                                                    <DropdownMenuItem onSelect={() => archiveProduct(product.id)} className="text-sm font-bold cursor-pointer focus:bg-amber-500/10 focus:text-amber-500">
                                                        <Archive className="w-4 h-4 mr-2" /> Archive
                                                    </DropdownMenuItem>
                                                ) : (
                                                    <DropdownMenuItem onSelect={() => unarchiveProduct(product.id)} className="text-sm font-bold cursor-pointer focus:bg-emerald-500/10 focus:text-emerald-500">
                                                        <Package className="w-4 h-4 mr-2" /> Unarchive
                                                    </DropdownMenuItem>
                                                )}
                                                <DropdownMenuItem
                                                    onSelect={async (e) => {
                                                        // Prevent default to prevent menu from closing immediately if we want, but letting it close is fine.
                                                        try {
                                                    ...
                                                    className="text-sm font-bold cursor-pointer text-rose-500 focus:bg-rose-500/10 focus:text-rose-500"
                                                >
