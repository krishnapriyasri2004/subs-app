...
export function SubscriptionLifecycleModal({
  ...
  const { mockCustomerSubscriptions, mockBusinessPlans, updateDocument } = useData();
  const subscription = mockCustomerSubscriptions.find((s) => s.id === subscriptionId);

  if (!subscription) return null;

  const currentPlan = mockBusinessPlans.find((p) => p.id === subscription.planId);

  const handleUpdateTier = async (newPlanId: string) => {
    try {
      await updateDocument('subscriptions', subscription.id, {
        planId: newPlanId,
        totalAmount: mockBusinessPlans.find(p => p.id === newPlanId)?.price || subscription.totalAmount,
      });
  ...
  const handleTogglePause = async () => {
    try {
      if (subscription.status === 'paused') {
        ...
        if (!pauseReason) {
          toast.error("Please provide a suspension justification.");
          return;
        }
        await updateDocument('subscriptions', subscription.id, {
          status: 'paused',
      ...
      <DialogContent className="max-w-3xl glass-card border-border/50 p-0 overflow-hidden outline-none">
                  ...
                  .map((plan) => (
                    ...
                    >
                      ...
                      <div className="mt-6 pt-6 border-t border-border/30 flex justify-between items-center">
                        ...
                        <Button size="sm" onClick={() => handleUpdateTier(plan.id)} className="rounded-lg h-9 bg-primary px-6 font-bold shadow-lg shadow-primary/20">Select Tier</Button>
                      </div>
                    </Card>
                  ))}
              </div>
                  ...
                  .map((plan) => (
                    ...
                    >
                      ...
                      <div className="pt-6 border-t border-border/30 flex justify-between items-center">
                        ...
                        <Button onClick={() => handleUpdateTier(plan.id)} variant="outline" size="sm" className="rounded-lg h-9 border-border/50 font-bold px-6">De-escalate</Button>
                      </div>
                    </Card>
                  ))}
              </div>
              ...
              {subscription.status !== 'paused' ? (
                ...
                <Card className="glass-card p-10 text-center space-y-6 animate-in-up">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-2 font-bold uppercase tracking-tighter">Halted on {subscription.pausedAt ? new Date(subscription.pausedAt).toLocaleDateString('en-GB') : 'Unknown'}</p>
                  </div>
        ...
        <div className="p-8 bg-muted/20 border-t border-border/30">
          <DialogFooter className="gap-3">
            ...
            {activeTab === 'pause' && (
              <Button onClick={handleTogglePause} className="rounded-xl h-12 px-10 bg-primary hover:bg-primary/90 text-primary-foreground font-black shadow-lg shadow-primary/20 flex items-center gap-2">
                {subscription.status === 'paused' ? 'Resume Continuously' : 'Suspend Operations'}
              </Button>
            )}
          </DialogFooter>
        </div>
