...
export function MySubscriptions() {
    const { user } = useAuth();
    const { mockCustomers, mockCustomerSubscriptions, mockBusinessPlans, updateDocument } = useData();
    const [isActionLoading, setIsActionLoading] = useState(false);

    const currentCustomer = mockCustomers.find(c => c.email === user?.email);

    const subsRaw = mockCustomerSubscriptions.filter(s => s.customerId === currentCustomer?.id);
    const subs = subsRaw.map(s => {
        const plan = mockBusinessPlans.find(p => p.id === s.planId);
        return {
            id: s.id,
            subNumber: s.id.toUpperCase(),
            name: plan?.name || 'Subscription',
            status: s.status as any,
            price: `₹${s.totalAmount}`,
            period: plan?.interval === 'monthly' ? 'Month' : 'Year',
            nextBilling: new Date(s.endDate).toLocaleDateString(),
            quantity: s.quantity || 1,
            color: 'indigo',
            vendorType: plan?.name.toLowerCase().includes('milk') || plan?.name.toLowerCase().includes('rice') ? 'daily_needs' : 'general' as any,
            deliveries: s.deliveries || [true, true, true, true, true, true, false]
        };
    });

    const updateSubStatus = async (id: string, status: Subscription['status']) => {
        const target = subsRaw.find(s => s.id === id);
        if (target) {
            await updateDocument('subscriptions', id, { ...target, status });
        }
    };
    ...
    const handleQuantity = async (id: string, delta: number) => {
        const sub = subsRaw.find(s => s.id === id);
        if (!sub) return;

        const newQuantity = Math.max(1, (sub.quantity || 1) + delta);
        if (newQuantity === (sub.quantity || 1)) return;

        try {
            await updateDocument('subscriptions', id, { ...sub, quantity: newQuantity });
    ...
    const toggleDeliveryDay = async (subId: string, dayIndex: number) => {
        const sub = subsRaw.find(s => s.id === subId);
        if (!sub) return;

        const currentDeliveries = sub.deliveries || [true, true, true, true, true, true, false];
        const newDeliveries = [...currentDeliveries];
        newDeliveries[dayIndex] = !newDeliveries[dayIndex];

        try {
            await updateDocument('subscriptions', subId, { ...sub, deliveries: newDeliveries });
