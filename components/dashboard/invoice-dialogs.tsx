...
export function IssueInvoiceDialog() {
    ...
    const { addDocument, mockTenants } = useData();
...
export function ViewInvoiceDialog({ invoice, entityName, trigger }: { invoice: any, entityName: string, trigger: React.ReactNode }) {
    const handleDownload = async () => {
        toast.loading(`Preparing PDF sequence for ${invoice.invoiceNumber}...`, { id: `download-${invoice.invoiceNumber}` });

        try {
            await downloadInvoicePdf(invoice, entityName);
            toast.success(`Downloaded ${invoice.invoiceNumber} PDF!`, { id: `download-${invoice.invoiceNumber}` });
        } catch (e) {
            toast.error("Failed to generate PDF.", { id: `download-${invoice.invoiceNumber}` });
        }
    };
        ...
        <Dialog>
            ...
            <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden bg-slate-50">
                ...
                <div className="p-6 bg-slate-50 flex justify-between items-center">
                    ...
                    <Button onClick={handleDownload} variant="outline" className="border-indigo-200 text-indigo-600 hover:bg-indigo-50 font-bold px-6">
