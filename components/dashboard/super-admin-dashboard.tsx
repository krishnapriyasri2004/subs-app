...
export default function SuperAdminDashboard() {
    ...
    const { mockTenants: tenants, addDocument, updateDocument, deleteDocument } = useData();
    ...
    const [rzpKeyId, setRzpKeyId] = useState('rzp_live_8xJk9lO2PqQ1w');
    const [rzpKeySecret, setRzpKeySecret] = useState('dummy_secret_key');

    const handleSaveSettings = async () => {
        try {
            await addDocument('platform_settings', {
                id: 'global_settings',
                gstRate,
                currency,
                fiscalYear,
            ...
            toast.success("Global Settings saved to Firebase successfully!");
        } catch (error) {
    ...
    const toggleTenantStatus = async (id: string, status: string) => {
        ...
        await updateDocument('tenants', id, { status: newStatus });
    };
    ...
    const handleUpdateTenantStatus = async (tenantId: string, newStatus: string, notes?: string) => {
        // Update tenant status
        await updateDocument('tenants', tenantId, { status: newStatus as Tenant['status'] });
        ...
        if (tenant) {
            ...
            await addDocument('approvalLogs', {
                ...newHistoryLog,
        ...
        <div className="space-y-10 pb-10 pt-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 animate-in-fade">
                ...
                <div className="flex gap-3">
                    <Button onClick={handleNewTenant} className="rounded-xl h-12 bg-primary text-primary-foreground font-black shadow-xl hover:bg-primary/90">
                                    ...
                                    {tenants.map((tenant) => {
                                        ...
                                        return (
                                            <TableRow key={tenant.id} className="border-border/50 hover:bg-muted/10">
                                                ...
                                                <TableCell className="text-right text-muted-foreground text-sm">{new Date(tenant.createdAt).toLocaleDateString()}</TableCell>
                                                <TableCell className="text-right space-x-2">
                                                    <Button variant="ghost" size="sm" onClick={() => handleEditTenant(tenant)} className="h-8 text-xs font-bold text-primary hover:text-primary hover:bg-primary/5">View</Button>
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="pt-5 border-t border-border/50 space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input type="text" placeholder="rzp_live_..." className="font-mono text-sm border-border/50 bg-background/50" value={rzpKeyId} onChange={(e) => setRzpKeyId(e.target.value)} />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Platform Key Secret</label>
                                            <Input type="password" placeholder="••••••••••••" className="font-mono text-sm border-border/50 bg-background/50" value={rzpKeySecret} onChange={(e) => setRzpKeySecret(e.target.value)} />
                                        </div>
                                        <Button className="w-full font-bold h-10 mt-2" onClick={handleSaveSettings}>Save Keys</Button>
                                    </div>
                                </div>
                            </div>
                        </Card>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-1 gap-8">
                        ...
                        <div className="space-y-8 flex flex-col">
                            ...
                            <Card className="glass-card flex flex-col">
                                ...
                                <div className="p-4 h-[250px] pt-6">
                                    <ResponsiveContainer width="100%" height="100%">
            ...
            <TenantDialog
                ...
                onSave={async (data) => {
                    if (editingTenant) {
                        try {
                            await updateDocument('tenants', editingTenant.id, {
                                ...data,
                        ...
                        try {
                            ...
                            toast.success(`Tenant ${data.name} initialized successfully!`);
                        } catch (error) {
