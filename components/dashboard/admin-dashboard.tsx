...
export default function AdminDashboard() {
  const {
    ...
    mockCustomerSubscriptions: subscriptions,
    mockDemoRequests,
    updateDocument,
    deleteDocument,
  } = useData();
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      await updateDocument('demo_requests', id, { status: newStatus });
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDocument('demo_requests', id);
    } catch (error) {
    ...
    <div className="space-y-10 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          label="Total Users"
          value={tenants.reduce((sum, t) => sum + t.currentUsersCount, 0)}
