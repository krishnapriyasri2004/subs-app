...
import { mockDemoRequests } from '@/lib/mock-data';
import {
  Users,
  Zap,
  TrendingUp,
...
export default function TenantDashboard() {
  const { user } = useAuth();
  const { mockTenants, mockInvoices: mockCustomerInvoices, mockCustomerSubscriptions, mockCustomers, mockBusinessPlans } = useData();
  const [demoRequests, setDemoRequests] = React.useState<any[]>(mockDemoRequests);

  React.useEffect(() => {
    const localRequests = JSON.parse(localStorage.getItem('subtrack_demo_requests') || '[]');
