...
import { Edit, Eye, MoreHorizontal, Download, IndianRupee, MessageSquare } from 'lucide-react';
                    ...
                    <TableBody>
                        {filteredInvoices.length === 0 ? (
                            ...
                            filteredInvoices.map((invoice) => {
                                const customerById = mockCustomers.find(c => c.id === (invoice as any).customerId);
                                const customerByName = mockCustomers.find(c => c.name === (invoice as any).customerName || c.name === (invoice as any).customer);
                                const customerByEmail = mockCustomers.find(c => c.email === invoice.customerEmail);
                                const customer = customerById || customerByName || customerByEmail;
                                    ...
                                    <TableCell>
                                        <div className="flex flex-col">
                                            <span className="font-bold text-slate-900">
                                                {(() => {
                                                    const name = customer?.name || invoice.customerName || (invoice as any).customer;
                                                    if (name && name !== 'Unknown' && name !== 'Unknown Customer' && name !== 'Customer') return name;
                                                    ...
                                                    const fallbackId = (invoice as any).customerId;
                                                    if (fallbackId && fallbackId.length > 5 && !fallbackId.includes('-')) {
                                                        return fallbackId.charAt(0).toUpperCase() + fallbackId.slice(1);
                                                    }
                                                    
                                                    return 'Subtrack Customer';
                                                })()}
                                            </span>
                                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{customerEmail || 'No email provided'}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell>
