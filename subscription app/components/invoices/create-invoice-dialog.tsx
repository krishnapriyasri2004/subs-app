...
export function CreateInvoiceDialog({ onInvoiceCreated }: { onInvoiceCreated?: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent, status: 'sent' | 'draft' = 'sent') => {
        ...
        try {
            console.log("Invoice Generation Started (Sub-folder):", { invoiceNumber, customerId: selectedCustomerId });
            ...
            // Save to localStorage (persistent mock)
            const existingInvoicesJson = localStorage.getItem('vendor_invoices');
            const existingInvoices = existingInvoicesJson ? JSON.parse(existingInvoicesJson) : [];
            localStorage.setItem('vendor_invoices', JSON.stringify([newInvoice, ...existingInvoices]));
            ...
            console.error("Invoice Generation Error (Sub-folder):", error);
