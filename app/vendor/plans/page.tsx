...
export default function VendorPlansPage() {
    ...
    const { mockBusinessPlans: plans, mockCoupons, addDocument, updateDocument, deleteDocument } = useData();
    ...
    const filteredPlans = plans.filter(plan => {
        // Primary check: Does this plan belong to this vendor (tenant)?
        if (user?.tenantId && plan.tenantId === user.tenantId) return true;

        // Secondary check for legacy/migration: Match by category
        const category = (plan.category || plan.type || '').toLowerCase();
        const vendorType = (user?.businessType || '').toLowerCase();

        if (vendorType === 'both') return true;
        if (vendorType && category === vendorType) return true;

        // If it's a generic plan and we have no specific matches, hide it from other vendors
        return false;
    });
    ...
    const handleOpenDialog = (plan: any = null) => {
        if (plan) {
            ...
            setFormData({
                ...
                price: typeof plan.price === 'string' ? plan.price.replace(/[₹,]/g, '') : String(plan.price),
                frequency: plan.frequency || (plan.interval === 'monthly' ? 'Month' : 'Year'),
    ...
    const handleCreatePlan = async () => {
        if (editingPlan) {
            ...
            await updateDocument('plans', editingPlan.id, {
                ...
                interval: formData.frequency === 'Month' ? 'monthly' : 'yearly',
            ...
            await addDocument('plans', {
                ...
                provider: user?.businessName || 'Local Vendor',
                providerType: user?.businessType || 'general', // Explicit link for discovery
        ...
        setOpen(false);
        setEditingPlan(null);
        setFormData({ name: '', description: '', price: '', frequency: 'Month', features: '', couponId: '' });
    };
                            ...
                            <div className="flex flex-col max-h-[90vh]">
                                ...
                                <div className="flex-1 overflow-y-auto px-10 py-2 custom-scrollbar">
                                    <div className="grid gap-8 py-4">
                                        ...
                                        <div className="grid gap-3">
                                            <Label htmlFor="coupon" className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-1">Promotional Coupon (Optional)</Label>
                                            <Select value={formData.couponId} onValueChange={(val) => setFormData({ ...formData, couponId: val })}>
                                                <SelectTrigger id="coupon" className="h-14 rounded-2xl border-slate-100 bg-slate-50/50 focus:bg-white transition-all font-bold px-6">
                                                    <SelectValue placeholder="No discount applied" />
                                                </SelectTrigger>
                                                <SelectContent className="rounded-xl border-slate-100">
                                                    <SelectItem value="none" className="font-bold">No Coupon</SelectItem>
                                                    {mockCoupons.map((coupon) => (
                                                        <SelectItem key={coupon.id} value={coupon.id} className="font-bold">
                ...
                {filteredPlans.map((plan) => (
                    <Card key={plan.id} className={cn(
                        ...
                        <div className="mb-8 flex items-baseline gap-1">
                            <span className="text-4xl font-black text-slate-900 tracking-tighter italic">₹{typeof plan.price === 'number' ? plan.price.toLocaleString('en-IN') : plan.price}</span>
                            <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">/ {plan.frequency || (plan.interval === 'monthly' ? 'Month' : 'Year')}</span>
                        </div>

                        {(() => {
                            const coupon = mockCoupons.find(c => c.id === plan.couponId);
                            if (!coupon || plan.couponId === 'none') return null;
                            return (
                                <div className="mb-6 p-3 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center justify-between group">
                                    <div className="flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center">
                                            <Tag className="w-4 h-4 text-emerald-600" />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Promo Active</p>
                                            <p className="text-xs font-black text-emerald-700">{coupon.code}</p>
                                        </div>
                                    </div>
                                    <span className="text-xs font-black text-emerald-600 bg-white px-2 py-1 rounded-md shadow-sm border border-emerald-50">
