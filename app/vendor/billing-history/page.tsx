...
const billingData = [
    { id: 'BIL-459201', date: '3 Mar 2026', period: 'Mar 01 - Mar 28', amount: '₹1,999', status: 'Paid', type: 'Subscription', invoice: 'INV-3240', invoiceId: 'inv-001' },
    { id: 'BIL-487826', date: '4 Mar 2026', period: 'Feb 01 - Feb 28', amount: '₹1,999', status: 'Paid', type: 'Subscription', invoice: 'INV-3112', invoiceId: 'inv-002' },
    { id: 'BIL-480535', date: '3 Mar 2026', period: 'Jan 01 - Jan 28', amount: '₹1,999', status: 'Paid', type: 'Subscription', invoice: 'INV-2998', invoiceId: 'inv-003' },
    { id: 'BIL-489256', date: '3 Mar 2026', period: 'Dec 01 - Dec 28', amount: '₹2,499', status: 'Paid', type: 'Overage', invoice: 'INV-2845', invoiceId: 'inv-001' },
    { id: 'BIL-491234', date: '4 Mar 2026', period: 'Nov 01 - Nov 28', amount: '₹1,999', status: 'Paid', type: 'Subscription', invoice: 'INV-2710', invoiceId: 'inv-002' },
    { id: 'BIL-501239', date: '3 Mar 2026', period: 'Oct 01 - Oct 28', amount: '₹1,999', status: 'Refunded', type: 'Subscription', invoice: 'INV-2550', invoiceId: 'inv-003' },
];

export default function BillingHistoryPage() {
    const { user } = useVendorAuth();
    const [searchTerm, setSearchTerm] = useState('');
    const [yearFilter, setYearFilter] = useState('Current Year (2026)');
    ...
    const filteredData = billingData.filter(b => {
        ...
        const matchesYear = yearFilter === 'Current Year (2026)' ? b.date.includes('2026')
            : yearFilter === 'Previous Year (2025)' ? b.date.includes('2025')
                : true;

        const matchesType = typeFilter === 'All Transactions' ? true : b.type === typeFilter;

        const matchesMonth = monthFilter === 'All Months' ? true : b.date.includes(monthFilter.substring(0, 3));
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                                ...
                                <DropdownMenuItem onClick={() => setYearFilter('Current Year (2026)')}>Current Year (2026)</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setYearFilter('Previous Year (2025)')}>Previous Year (2025)</DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>

                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" className="h-9 px-3 text-xs bg-slate-50 font-bold border-slate-100 text-slate-600 rounded-md whitespace-nowrap">
                                    {monthFilter} <ChevronDown className="w-3.5 h-3.5 ml-2 text-slate-400" />
                                </Button>
                            </DropdownMenuTrigger>
                            ...
                            {filteredData.map((bill) => (
                                <tr key={bill.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-4 py-4 text-right">
                                        <Link href={`/vendor/invoices/${bill.invoiceId || 'inv-001'}`}>
                                            <Button
                                                variant="ghost"
                                                size="sm"
                                                className="h-8 px-2 text-indigo-600 font-bold hover:bg-indigo-50 transition-opacity flex items-center gap-1.5 ml-auto cursor-pointer"
                                            >
