...
export default function InvoicesPage() {
  ...
  const { mockInvoices: allMockInvoices, mockTenants, mockCustomers } = useData();

  const [allInvoices, setAllInvoices] = useState<any[]>([]);

  const fetchInvoices = () => {
    const localInvoices = JSON.parse(localStorage.getItem('admin_invoices') || '[]');
    setAllInvoices([...localInvoices, ...allMockInvoices]);
  };
  ...
  const invoices = isPlatformAdmin
    ? allInvoices
    : isBusinessOwner
      ? allMockInvoices.filter(i => i.tenantId === user?.tenantId)
  ...
  const handleDownload = async (invoice: any) => {
    toast.loading(`Preparing PDF for ${invoice.invoiceNumber}...`, { id: 'download' });

    try {
      await downloadInvoicePdf(invoice, getEntityName(invoice));
      toast.success(`Downloaded ${invoice.invoiceNumber} successfully!`, { id: 'download' });
    } catch (e) {
                  ...
                  <td className="py-5 px-6">
                    <div className="flex gap-2 justify-end transition-opacity">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => router.push(`/dashboard/customers?tenantId=${invoice.tenantId}`)}
