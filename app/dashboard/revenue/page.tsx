    ...
    const handleExport = () => {
        toast.promise(
            new Promise(async (resolve) => {
                ...
                // Create CSV Content
                const csvRows = [
                    ['Month', 'Revenue', 'Expansion', 'Churn'],
                    ...revenueOverTime.map(row => [row.month, row.revenue, row.expansion, row.churn])
                ];
                ...
                a.setAttribute('download', `platform_revenue_ledger_${new Date().toISOString().split('T')[0]}.csv`);
            ...
            }),
            {
                ...
                success: 'Ledger download initialized!',
                ...
                <div className="flex gap-3">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="rounded-xl h-12 border-border/50 font-bold min-w-[140px]">
                                <Filter className={cn("w-4 h-4 mr-2", selectedSegment !== 'All Segments' && "text-primary")} />
                                {selectedSegment}
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-56 rounded-xl p-2">
                            <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground px-2 py-1.5">Filter by Tier</DropdownMenuLabel>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setSelectedSegment('All Segments')}>All Segments</DropdownMenuItem>
                            <DropdownMenuSeparator className="my-1" />
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setSelectedSegment('Enterprise')}>Enterprise Tier</DropdownMenuItem>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setSelectedSegment('Professional')}>Professional Tier</DropdownMenuItem>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setSelectedSegment('Basic')}>Basic Tier</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>

                    <Button onClick={handleExport} className="rounded-xl h-12 bg-primary text-primary-foreground font-black shadow-lg shadow-primary/20">
