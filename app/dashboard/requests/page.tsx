...
export default function RequestsPage() {
    const { mockDemoRequests: requests, updateDocument, deleteDocument } = useData();
    ...
    const handleStatusChange = async (id: string, newStatus: string) => {
        try {
            await updateDocument('demoRequests', id, { status: newStatus });
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteDocument('demoRequests', id);
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48 rounded-xl p-2">
                            <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground px-2 py-1.5">Filter by Status</DropdownMenuLabel>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setStatusFilter('All')}>All Requests</DropdownMenuItem>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setStatusFilter('pending')}>Pending</DropdownMenuItem>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setStatusFilter('scheduled')}>Scheduled</DropdownMenuItem>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setStatusFilter('completed')}>Completed</DropdownMenuItem>
                            <DropdownMenuItem className="rounded-lg font-bold text-xs cursor-pointer" onClick={() => setStatusFilter('rejected')}>Rejected</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
            </div>
