...
import { TransactionAuditDialog } from '@/components/dashboard/transaction-audit-dialog';
...
export default function PaymentLogsPage() {
    ...
    const logs: any[] = isPlatformAdmin
        ...
        })).sort((a: any, b: any) => b.issueDate.getTime() - a.issueDate.getTime())
    ...
    const filteredLogs = logs.filter(log =>
        ...
        (log.razorpayPaymentId && log.razorpayPaymentId.toLowerCase().includes(searchTerm.toLowerCase()))
    );

    const handleAction = (message: string, type: 'success' | 'info' | 'loading' = 'info') => {
        if (type === 'success') toast.success(message);
        else if (type === 'loading') toast.loading(message, { duration: 2000 });
    ...
    const handleDownloadCSV = () => {
        toast.promise(
            new Promise((resolve) => {
                setTimeout(() => {
                    const headers = ['Settlement Node', 'Transaction ID', 'Entity Name', 'Amount', 'Status', 'Date'];
                    const csvRows = [headers.join(',')];

                    filteredLogs.forEach(log => {
                        const amount = log.totalAmount;
                        const status = log.status === 'paid' ? 'Settled' : log.status === 'sent' ? 'Processing' : 'Failed';
                        const date = new Date(log.issueDate).toLocaleDateString();
                        const txnId = log.razorpayPaymentId || `TXN_${log.id.slice(-6).toUpperCase()}`;
                        const entityName = log.entityName.replace(/,/g, '');

                        csvRows.push(`${log.invoiceNumber},${txnId},${entityName},${amount},${status},${date}`);
                    });
                    ...
                    link.setAttribute("download", `daily_settlements_${new Date().getTime()}.csv`);
            ...
            {
                loading: 'Exporting full daily settlements to CSV...',
                success: 'Daily settlements exported successfully',
                error: 'Export failed'
            }
        );
    };
    ...
    return (
        <div className="space-y-10 animate-in-up pb-10">
                ...
                <div className="flex gap-3">
                    <Dialog>
                        <DialogTrigger asChild>
                            <Button variant="outline" className="rounded-xl h-12 border-border/50 font-bold hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200">
                        ...
                        <DialogContent className="sm:max-w-md">
                            <DialogHeader>
                                <DialogTitle className="flex items-center gap-2">
                                    <ShieldCheck className="w-5 h-5 text-emerald-500" /> System Operational
                                </DialogTitle>
                                <DialogDescription>
                                    All payment gateway nodes are functioning perfectly with 99.99% uptime.
                                </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4 py-4">
                                <div className="flex justify-between items-center bg-muted/30 p-4 rounded-lg border border-border/50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="font-bold text-sm">Credit Cards Network</span>
                                    </div>
                                    <span className="text-xs font-bold text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded">17ms ping</span>
                                </div>
                                <div className="flex justify-between items-center bg-muted/30 p-4 rounded-lg border border-border/50">
                                    <div className="flex items-center gap-3">
                                        <div className="bg-emerald-500/10 p-2 rounded-full">
                                        ...
                                        <span className="font-bold text-sm">Netbanking Nodes</span>
                                    </div>
                                    <span className="text-xs font-bold text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded">24ms ping</span>
                                </div>
                                <div className="flex justify-between items-center bg-muted/30 p-4 rounded-lg border border-border/50">
                                    <div className="flex items-center gap-3">
                                        <div className="bg-emerald-500/10 p-2 rounded-full">
                                        ...
                                        <span className="font-bold text-sm">UPI Settlements</span>
                                    </div>
                                    <span className="text-xs font-bold text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded">12ms ping</span>
                                </div>
                            </div>
                        </DialogContent>
                    </Dialog>
                    <Button onClick={handleDownloadCSV} className="rounded-xl h-12 bg-foreground text-background font-black shadow-xl hover:bg-slate-800">
            ...
            {/* Pagination / Footer */}
            <div className="flex flex-col md:flex-row justify-between items-center bg-muted/20 p-6 rounded-3xl border border-border/50 gap-4">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Displaying 1-{filteredLogs.length} of {logs.length} Settlements</p>
                <div className="flex gap-2">
                    <Button onClick={() => handleAction('Loading previous records...')} variant="outline" size="sm" className="rounded-xl h-10 px-6 font-bold hover:bg-muted/50 transition-colors">Previous</Button>
                    <Button onClick={() => handleAction('Loading next page of transaction records...')} variant="outline" size="sm" className="rounded-xl h-10 px-6 font-bold bg-background shadow-sm hover:text-primary transition-colors">Next</Button>
                </div>
            </div>
        </div>
    );
