...
import { SubscriptionAddDialog } from '@/components/subscriptions/subscription-add-dialog';
...
export default function SubscriptionsPage() {
  ...
  const isBusinessOwner = user?.role === 'tenant_owner' || (user?.role === 'admin' && !!user?.tenantId);
  ...
  const { mockCustomerSubscriptions, mockTenants, mockCustomers, mockBusinessPlans, mockSubscriptionPlans } = useData();
  ...
  const subscriptionsToDisplay = isPlatformAdmin
    ...
    : isBusinessOwner
      ...
      : mockCustomerSubscriptions.filter(s => s.customerId === user?.id); // Filter for the logged in customer
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 animate-in-fade">
        ...
        {!isCustomer && (
          <div className="flex gap-3">
            ...
            <Button onClick={() => setIsAddModalOpen(true)} className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/20 rounded-xl px-6 h-12">
        ...
        <TabsContent value="subscriptions" className="space-y-6">
          <div className="grid gap-6">
            {subscriptionsToDisplay.map((subscription, index) => {
              const tenant = mockTenants.find((t) => t.id === (subscription as any).tenantId);
              const customer = mockCustomers.find((c) => c.id === (subscription as any).customerId);
              const plan = [...mockBusinessPlans, ...mockSubscriptionPlans].find((p) => p.id === subscription.planId);

              const displayName = customer?.name || tenant?.name || 'Managed Service';
                    ...
                    <div className="flex flex-col lg:flex-row gap-6">
                      ...
                      <div className="flex-1 min-w-[300px]">
                        <div className="flex items-center gap-4 mb-4">
                          <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-bold text-xl group-hover:scale-110 transition-transform uppercase">
                            {displayName.charAt(0)}
                          </div>
                          <div>
                            <h3 className="text-xl font-bold text-foreground group-hover:text-primary transition-colors">{displayName}</h3>
                            {isPlatformAdmin && businessName && (
                              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest -mt-1">{businessName}</p>
                            )}
                        ...
                        <div className="grid grid-cols-2 gap-6 mt-6 pt-6 border-t border-border/50">
                          <div>
                            ...
                            <div className="flex items-center gap-1.5 font-bold text-foreground">
                              ...
                              ₹{(subscription.totalAmount || 0).toLocaleString()}
                            </div>
                          </div>
                          <div>
